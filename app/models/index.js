...
Sequelize.prototype.$$defineModel = function (modelName = '', fields = {}, config = {}) {
    const model = this.define(snakeCase(modelName), { ...getFields(Sequelize.DataTypes, fields) }, {
        createdAt: 'createdAt',
        updatedAt: 'updatedAt',
    ...
    model.__proto__.$$findOne = function ({ query = {}, options = {}, throwError = true, error = '' }) {
        return this.findOne(query, options).then((res) => {
            if (!res && throwError) {
                return Promise.reject({
                    error: error || $translate(Errors.Models.NotFound, {
    ...
    model.__proto__.$$findByPk = function ({ id, throwError = true, error = '' }) {
...
sequelize
    .authenticate()
    .then(() => global.log.info('Database connection has been established successfully.'))
    .done();
